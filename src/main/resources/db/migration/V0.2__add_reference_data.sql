...
INSERT INTO recyclable_status_translation(id, recyclable_status_id, translation_locale_id, text, description) SELECT UUID(), r.id, t.id, 'à recycler si vide', 'peut être recyclé une fois vidé (sac jaune ou bac de tri)' FROM translation_locale t, recyclable_status r WHERE t.name = 'fr' AND r.name = 'recyclable-if-empty';
